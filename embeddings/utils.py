...
def get_embedding(str_dataset:str,embedding_size:int=100,overwrite=False):
    ...
    if file_not_found or overwrite:
        with  io.open("embeddings_data\\"+str_dataset,"r", encoding="utf8") as f_e:
                for i,line in enumerate(f_e):
                    try:
                        ...
                        arr_temp = arr_line[-embedding_size:]
                        flag = False
                        
                        
                        dict_embedding[word] = np.array(arr_temp, dtype=np.float16)
                        #if line.startswith("principe"):
                            #print("Palavra principe:"+word)
                            #print(arr_line)
